...
이러한 문제를 해결하기 위해 **verifier-free RL** 방법론이 등장함. 대표적으로 RLPR은 reference 정답의 likelihood를 활용하여 보상을 정의함. 하지만 이 또한 reference answer에 대한 과적합이라는 한계를 보임 모델이 정답의 의미를 이해하기보다 **정확히 같은 표현을 재현하는 방향으로 수렴**하는 문제가 발생.
...
- Reinforcement Learning with Reference Probability Reward (RLPR)
    ...
    RLPR은 기존의 RLVR이 0 또는 1의 이진 reward를 주는 것과 달리, Verifier 없이도 dense한 0~1 사이의 reward를 주는 것을 목표로 한다. 이를 위해서 RLPR은 input $x$에 대해서 생성된 추론 과정 $z$와 응답 $y$에 대해서 reference 정답 $y^*$의 생성 확률을 통해 계산된 리워드 $\hat r$을 부여한다. 이때 $\hat r$은 추론 과정에 대한 영향만 고려하기 위하여 다음과 같이 계산된다. 이때 $f(y^*|x)$는 input x에 대해서 output $y^*$가 나올 확률의 per-token probability의 aggregation 함수이다.
